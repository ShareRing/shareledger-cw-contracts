use crate::{
    ...
    msg::{
        CheckRoyaltiesResponse, Cw1155RoyaltiesExecuteMsg, Cw1155RoyaltiesQueryMsg, InstantiateMsg,
...
use cw1155::{
    ...
    Cw1155ReceiveMsg, IsApprovedForAllResponse, TokenInfoResponse, TokensResponse,
};

use cw_utils::Expiration;

#[test]
fn auto_increment_mint() {
    ...
    let rs = query(
        deps.as_ref(),
        mock_env(),
        Cw1155RoyaltiesQueryMsg::CheckRoyalties {},
    )
    .unwrap();

    assert!(
        from_binary::<CheckRoyaltiesResponse>(&rs)
            .unwrap()
            .royalty_payments
    );

    let msg = Cw1155RoyaltiesExecuteMsg::Mint {
        to: user1.clone(),
        token_id: None,
        value: 1u64.into(),
        royalty_percentage: Some(50),
        royalty_payment_address: Some(user2.clone()),
        msg: None,
    };
    ...
    let rs = query(
        ...
        Cw1155RoyaltiesQueryMsg::RoyaltyInfo {
            ...
            sale_price: 100u64.into(),
        },
    )
    .unwrap();

    let rs = from_binary::<RoyaltiesInfoResponse>(&rs).unwrap();

    assert_eq!(rs.royalty_amount, &50u64.into());
    assert_eq!(rs.address, user2.clone());
}
...
fn check_transfers() {
    ...
    let mint_msg = Cw1155RoyaltiesExecuteMsg::Mint {
        to: user1.clone(),
    ...
    assert_eq!(
        ...
        query(
            deps.as_ref(),
            mock_env(),
            Cw1155RoyaltiesQueryMsg::Balance {
                owner: user1.clone(),
    ...
    let transfer_msg = Cw1155RoyaltiesExecuteMsg::SendFrom {
        from: user1.clone(),
        to: user2.clone(),
        token_id: token1.clone(),
        value: 1u64.into(),
    ...
    execute(
        ...
        mock_env(),
        mock_info(user1.as_ref(), &[]),
        Cw1155RoyaltiesExecuteMsg::ApproveAll {
    ...
    assert_eq!(
        execute(
            ...
            mock_env(),
            mock_info(minter.as_ref(), &[]),
            Cw1155RoyaltiesExecuteMsg::BatchMint {
                to: user2.clone(),
    ...
    let batch_transfer_msg = Cw1155RoyaltiesExecuteMsg::BatchSendFrom {
        from: user2.clone(),
        to: user1.clone(),
        batch: vec![
    ...
    assert_eq!(
        query(
            deps.as_ref(),
            mock_env(),
            Cw1155RoyaltiesQueryMsg::BatchBalance {
                owner: user1.clone(),
    ...
    execute(
        ...
        mock_env(),
        mock_info(user1.as_ref(), &[]),
        Cw1155RoyaltiesExecuteMsg::RevokeAll {
            operator: minter.clone(),
        },
    )
    .unwrap();
    ...
    assert_eq!(
        query(
            deps.as_ref(),
            mock_env(),
            Cw1155RoyaltiesQueryMsg::IsApprovedForAll {
                owner: user1.clone(),
                operator: minter.clone(),
            }
        ),
    ...
    assert_eq!(
        execute(
            ...
            mock_env(),
            mock_info(user1.as_ref(), &[]),
            Cw1155RoyaltiesExecuteMsg::BatchBurn {
                from: user1.clone(),
                batch: vec![(token2.clone(), 1u64.into()), (token3.clone(), 1u64.into())]
            }
        )
...
fn check_queries() {
    ...
    assert_eq!(
        query(
            deps.as_ref(),
            mock_env(),
            Cw1155RoyaltiesQueryMsg::AllTokens {
                start_after: Some("token5".to_owned()),
                limit: Some(5),
            },
        ),
    ...
    assert_eq!(
        query(
            deps.as_ref(),
            mock_env(),
            Cw1155RoyaltiesQueryMsg::TokenInfo {
...
fn approval_expires() {
    ...
    let query_msg = Cw1155RoyaltiesQueryMsg::IsApprovedForAll {
        owner: user1,
        operator: user2,
    };
    assert_eq!(
